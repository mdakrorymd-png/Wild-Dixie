...
**`https://www.wilddixie.com/` could not be reached from this session** — outbound access to that domain is blocked by the environment's network proxy (`EGRESS_BLOCKED`/`403` on the CONNECT tunnel), and it returns no results in web search, meaning it is not indexed. This is a sandbox network restriction, not evidence about the site itself.

**Correction (per the founder, 2026-08-17): the domain is real and purchased** — my earlier reading of `DEPLOY.md`'s wording as "not yet purchased" was wrong; that line describes the *general* option to buy a custom domain, not this project's actual status. A DNS lookup from this session confirms it: `wilddixie.com` and `www.wilddixie.com` both resolve, and **both point to Vercel** (`cname.vercel-dns.com`), not to the free-tier Render blueprint described in this repo's own `render.yaml`/`DEPLOY.md`. That means the actual production deployment is likely a **separate Vercel project** not fully reflected in this repo's deploy docs — **UNKNOWN/DATA REQUIRED**: whether that Vercel deployment builds from this same repo, whether the FastAPI backend is hosted somewhere production-grade (Vercel doesn't run long-lived Python backends the way this project's `backend/` is structured), and whether the P0 fixes pushed to `docs/wild-dixie-v2-audit.md`'s companion commit are live there yet. **The free-tier-Render infrastructure risk in §20 should be treated as a risk about the `render.yaml` path specifically, not a confirmed statement about what's live today** — re-verify against the real Vercel/backend setup before treating it as resolved either way.

Every other claim in this report about "the website" is about **the codebase that would ship as the website**, which remains the strongest available evidence and — per the instruction to never invent data — the honest thing to audit.
